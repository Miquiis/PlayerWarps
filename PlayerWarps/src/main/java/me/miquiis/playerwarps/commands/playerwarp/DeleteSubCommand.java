...
public class DeleteSubCommand extends mSubCommand {

    private final ConfigManager configManager;
    private final MessagesManager messagesManager;
    private final WarpManager warpManager;

    public DeleteSubCommand(PlayerWarps plugin)
    {
    ...
    public String getDescription() {
        return "Deletes a specific warp.";
    }

    @Override
    public String getSyntax() {
        return "/playerwarp delete <warpname>";
    }
    ...
    public void perform(mCommand main, CommandSender sender, ArrayList<String> args) {
        if (args.size() == 1)
        {
            ...
            if (warpManager.removePlayerWarp(warpName))
            {
                player.sendMessage(messagesManager.getMessage("warp-delete").replace("%WARPNAME%", warpName));
            }
            else
            {
